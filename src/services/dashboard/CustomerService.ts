...
export class CustomerService {
  ...
  async getCustomerWithOrders(outletId: string, id: string, limit = 20) {
    return this.customerRepo.findByIdWithOrders(outletId, id, limit);
  }
  ...
  async deleteCustomer(outletId: string, id: string) {
    ...
    if (existing._count?.orders && existing._count.orders > 0) {
      throw new Error(
        "Tidak dapat menghapus pelanggan yang memiliki riwayat order.",
      );
    }
