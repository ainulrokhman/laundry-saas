...
export const GET = withAuth(
    async (request: Request, session: ExtendedSession, { params }: { params: Promise<{ id: string }> }) => {
        try {
            ...
            const customer = await customerService.getCustomer(session.outletId!, id);
...
export const PUT = withAuth(
    async (request: Request, session: ExtendedSession, { params }: { params: Promise<{ id: string }> }) => {
        try {
            ...
            const customer = await customerService.updateCustomer(session.outletId!, id, sanitizedData);
            ...
            return Response.json(
                {
                    success: false,
                    error: error instanceof Error ? error.message : 'Gagal memperbarui data pelanggan',
                },
                { status: 500 }
            );
        }
